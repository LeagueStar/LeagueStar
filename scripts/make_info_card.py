...
FIELDS = [
    ...
    ("Stack",      "JavaScript, HTML/CSS, GCP, Terraform, K8s"),
    ("Highlights", "GCP badges: Compute Engine, VPC, GKE"),
    ("Contact",    "linkedin.com/in/kartik-devdhawala"),
]
COLORS = {
    "key": "#39d353",
    "value": "#c9d1d9",
...
def build_footer(y):
    """A small pulsing dot that loops forever after the fields finish
    fading in - keeps the card visibly 'alive' whenever someone views
    the profile, not just in the ~1.5s window right after page load."""
    return f'''
    <circle cx="{PAD_X + 4}" cy="{y}" r="4" fill="{COLORS['dot_green']}">
      <animate attributeName="opacity" values="1;0.25;1" dur="1.6s" repeatCount="indefinite" />
    </circle>
    <text x="{PAD_X + 16}" y="{y + 4}" font-family="{FONT}" font-size="11"
          fill="{COLORS['key']}">live</text>'''


def main():
    height = TITLE_H + 34 + len(FIELDS) * LINE_H + 34

    rows = "".join(row_svg(i, k, v) for i, (k, v) in enumerate(FIELDS))
    footer_y = TITLE_H + 34 + len(FIELDS) * LINE_H + 14
    footer = build_footer(footer_y)
