...
def main():
    ...
    last_week_idx, last_day_idx = len(weeks) - 1, None
    ...
    for w_idx, week in enumerate(weeks):
        for d_idx, cell in enumerate(week):
            ...
            if w_idx == last_week_idx:
                last_day_idx = d_idx  # track the most recent day for the pulse
    ...
    # persistent pulse on today's cell - a static heatmap that finished
    # revealing 2 seconds ago just looks like a picture; this keeps a
    # small piece of it visibly "live" no matter when someone looks.
    pulse_svg = ""
    if last_day_idx is not None:
        px = LEFT_PAD + last_week_idx * (CELL + GAP) - 2
        py = TOP_PAD + last_day_idx * (CELL + GAP) - 8
        pulse_delay = round((max_delay_steps / max_delay_steps) * 1.1 + 0.3, 3)
        pulse_svg = (
            f'<rect x="{px}" y="{py}" width="{CELL + 4}" height="{CELL + 4}" '
            f'rx="4" ry="4" fill="none" stroke="{PALETTE[-1]}" stroke-width="1.5" opacity="0">'
            f'<animate attributeName="opacity" values="0;0.9;0" dur="1.8s" '
            f'begin="{pulse_delay}s" repeatCount="indefinite" />'
            f'</rect>'
        )
